...
export class Game2Component {
  lanes = ['A', 'B', 'C']; // Carriles disponibles
  currentLane = 1; // Carril inicial
  targetLane = 0; // Carril objetivo
  gameActive = true;
  level = 1;
  ...
  // Validar si el carril seleccionado es correcto
  checkLane(selectedLetter: AudioGame) {
    if (this.targetLetter && selectedLetter.letter === this.targetLetter.letter) {
      this.points += 10;
      this.progress += 10;
      this.feedback = `¡Correcto! La letra es "${this.options[this.targetLane].letter}"`;
      Swal.fire('¡Bien hecho!', 'Puntaje ganado: 10', 'success');
    } else {
      this.points -= 5;
      this.progress = Math.max(0, this.progress - 5);
      this.feedback = '¡Incorrecto! Inténtalo de nuevo.';
      Swal.fire('Ups...', 'Carril incorrecto.', 'error');
    }

    if (this.progress >= 100) {
      this.gameActive = false;
      Swal.fire('¡Ganaste!', `Puntaje final: ${this.points}`, 'success');
    } else {
      this. startNewLevel();
    }
  }
